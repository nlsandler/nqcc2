...
module type CFG = sig
  ...
  val update_basic_block : int -> 'a basic_block -> 'a t -> 'a t
    ...
    let label_map =
      List.fold_left
        (fun lbl_map (_, blk) ->
          ...
          | Label lbl -> String_map.add lbl blk.id lbl_map
          | _ -> lbl_map)
        String_map.empty g.basic_blocks
    in

    (* add outgoing edges from a single basic block *)
    let process_node (id_num, block) =
      let next_block =
        if id_num = fst (Utils.last g.basic_blocks) then Exit
        else Block (id_num + 1)
      in
      let (), last_instr = Utils.last block.instructions in

      match Instr.simplify last_instr with
      | Return -> add_edge block.id Exit g
      | UnconditionalJump target ->
          let target_id = String_map.find target label_map in
