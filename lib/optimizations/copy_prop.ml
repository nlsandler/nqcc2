...
(* be careful to always use our own eq/compare instead of built-in one b/c 0.0
   and -0.0 compare equal, and NaN doesn't equal itself, using (=) *)
  ...
  if !Settings.debug then
    let copies_printer fmt copies =
      ...
      Format.pp_print_list
        ...
        pp_cp fmt
        (ReachingCopies.elements copies);
    ...
    let new_copies =
      ...
      | Tacky.Copy { src; dst } ->
          if ReachingCopies.mem { src = dst; dst = src } current_copies then
            ...
            (* if types are the same, kill dst but don't count this as a
               reaching copy *)
