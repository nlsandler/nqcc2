...
let rec optimize_fun debug_label (opts : Settings.optimizations) = function
  | [] -> []
  | instructions ->
      let aliased_vars = Address_taken.analyze instructions in
      (* run the optimization pipeline *)
      let constant_folded =
        if opts.constant_folding then
          Constant_folding.optimize debug_label instructions
        else instructions
      in
      let cfg = TackyCfg.instructions_to_cfg debug_label constant_folded in
      let cfg1 =
      ...
      else optimize_fun debug_label opts optimized_instructions
...
let optimize opts src_file tacky_program =
  ...
  let handle_tl = function
    | Function f ->
        let debug_lbl = Filename.chop_extension src_file ^ "_" ^ f.name in
        Function { f with body = optimize_fun debug_lbl opts f.body }
    | other_tl -> other_tl
  in
