...
let rec optimize_fun (opts : Settings.optimizations) = function
  | [] -> []
  | instructions ->
      let aliased_vars = Address_taken.analyze instructions in
      (* run the optimization pipeline *)
      let constant_folded =
        if opts.constant_folding then Constant_folding.optimize instructions
        else instructions
      in
      let cfg = TackyCfg.instructions_to_cfg constant_folded in
      let cfg1 =
      ...
      else optimize_fun opts optimized_instructions

let optimize opts tacky_program =
  let open Tacky in
  let (Program tls) = tacky_program in
  let handle_tl = function
    | Function f -> Function { f with body = optimize_fun opts f.body }
    | other_tl -> other_tl
  in
