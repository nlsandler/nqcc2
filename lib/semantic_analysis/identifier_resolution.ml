...
let rec resolve_statement id_map = function
  ...
  | LabeledStatement (lbl, stmt) ->
      LabeledStatement (lbl, resolve_statement id_map stmt)
  | Goto lbl -> Goto lbl
  ...
  | Switch s ->
      Switch
        {
          s with
          control = resolve_exp id_map s.control;
          body = resolve_statement id_map s.body;
        }
  | Case (v, stmt, id) -> Case (v, resolve_statement id_map stmt, id)
  | Default (stmt, id) -> Default (resolve_statement id_map stmt, id)
...
and resolve_function_declaration id_map fn =
  ...
  | _ ->
      ...
      in
      let new_map = Map.add fn.name new_entry id_map in
...
let resolve_file_scope_variable_declaration id_map
    ({ name; _ } as vd : initializr variable_declaration) =
  let new_map =
    Map.add name
      { unique_name = name; from_current_scope = true; has_linkage = true }
      id_map
