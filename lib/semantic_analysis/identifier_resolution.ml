...
let copy_struct_map m =
  (* return a copy of the map with from_current_block set to false for every entry *)
  Map.map (fun entry -> { entry with struct_from_current_scope = false }) m

(* replace structure tags in type specifiers *)
let rec resolve_type struct_map = function
  | Structure tag -> (
      try
        let unique_tag = Map.(struct_map --> tag).unique_tag in
...
let rec resolve_exp struct_map id_map = function
  ...
  | CompoundAssignment (op, left, right) ->
      ...
      CompoundAssignment
        ( op,
          resolve_exp struct_map id_map left,
          resolve_exp struct_map id_map right )
  | PostfixIncr e -> PostfixIncr (resolve_exp struct_map id_map e)
  | PostfixDecr e -> PostfixDecr (resolve_exp struct_map id_map e)
  | Var v -> (
      (* rename var from map  *)
...
let rec resolve_statement struct_map id_map = function
  ...
  | LabeledStatement (lbl, stmt) ->
      LabeledStatement (lbl, resolve_statement struct_map id_map stmt)
  | Goto lbl -> Goto lbl
  ...
  | Switch s ->
      Switch
        {
          s with
          control = resolve_exp struct_map id_map s.control;
          body = resolve_statement struct_map id_map s.body;
        }
  | Case (v, stmt, id) -> Case (v, resolve_statement struct_map id_map stmt, id)
  | Default (stmt, id) -> Default (resolve_statement struct_map id_map stmt, id)
...
and resolve_structure_declaration struct_map { tag; members } =
  let prev_entry = Map.find_opt tag struct_map in
  let new_map, resolved_tag =
    ...
    | _ ->
        ...
        (Map.(struct_map <-- (tag, entry)), unique_tag)
  in
