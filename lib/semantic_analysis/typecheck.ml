...
let validate_struct_definition U.{ tag; members } =
  ...
  if Type_table.mem tag then failwith "Structure was already declared"
    ...
    let member_names = ref StringSet.empty in
    let validate_member U.{ member_name; member_type } =
      if StringSet.mem member_name !member_names then
        failwith
          ("Duplicate declaration of member "
          ^ member_name
          ^ " in structure "
          ^ tag)
      else member_names := StringSet.add member_name !member_names;
...
let typecheck_struct_decl (U.{ tag; members } as sd) =
  if members = [] then (* ignore forward declarations *) ()
  else (
    ...
    let build_member_def (current_size, current_alignment, current_members)
      ...
      let new_members =
        StringMap.add member_name member_entry current_members
      in
      (new_size, new_alignment, new_members)
    in
    let unpadded_size, alignment, member_defs =
      List.fold_left build_member_def (0, 1, StringMap.empty) members
    in
  ...
  let cvt U.{ member_name; member_type } = T.{ member_name; member_type } in
  T.{ tag; members = List.map cvt members }
...
and typecheck_dot_operator strct member =
  ...
  | Types.Structure tag ->
      (* typecheck_and_convert already validated that this structure type is
         complete *)
      let struct_def = Type_table.find tag in
      let member_typ =
        try (StringMap.find member struct_def.members).member_type
        with Not_found ->
...
let rec static_init_helper var_type init =
  ...
  | Structure tag, U.CompoundInit inits ->
      ...
      if List.length inits > List.length members then
        ...
        in
        let initialized_members = ListUtil.take (List.length inits) members in
...
and typecheck_local_var_decl { name; init; storage_class; var_type } =
  ...
  | Some Static ->
      ...
      (* NOTE: we won't actually use init in subsequent passes so we can drop
         it*)
      T.{ name; init = None; storage_class; var_type }
  | None ->
      ...
      T.
        {
          ...
          var_type;
          storage_class;
          init = opt_typecheck (typecheck_init var_type) init;
        }
