...
let copy_bytes_to_reg src_val dst_reg ~byte_count =
  ...
  in
  (* [0; 1; ... ; byte_count - 1] *)
  let byte_counts = List.init byte_count Fun.id |> List.rev in
    ...
    if i < byte_count - 1 then
      Assembly.
        [
          mv;
          Binary
            { op = ShrBinop; t = Quadword; src = Imm 8L; dst = Reg src_reg };
        ]
    else [ mv ]
  in
  ...
  if size > 16 then
    ...
    let rec f = function
      ...
      | Array { elem_type; size } ->
          List.flatten (ListUtil.make_list size (f elem_type))
      | t -> [ t ]
    in

    let scalar_types = f (Structure tag) in
    let first, last = (List.hd scalar_types, List.(hd (rev scalar_types))) in
    if size > 8 then
...
let classify_structure tag =
  match Hashtbl.find_opt classified_structures tag with
    ...
    | Structure s ->
        ...
        let updated_int, updated_dbl, use_stack =
          if List.hd classes = Mem then
            ...
            let _, tentative_ints, tentative_dbs =
              List.fold_left process_one_eightbyte
        ...
        let updated_stack_args =
          if use_stack then
            ...
            List.fold_left add_stack_args stack_args
...
let classify_return_type = function
  ...
  | t ->
      ...
      if return_on_stack then ([ Assembly.AX ], true)
        ...
        let int_regs = ListUtil.take (List.length ints) Assembly.[ AX; DX ] in
        let dbl_regs =
          ListUtil.take (List.length dbls) Assembly.[ XMM0; XMM1 ]
        in
...
let convert_function_call f args dst =
  ...
  let pass_int_reg_arg idx (arg_t, arg) =
    let r = List.nth int_param_passing_regs (idx + first_intreg_idx) in
    match arg_t with
  ...
  let pass_dbl_reg_arg idx arg =
    let r = List.nth dbl_param_passing_regs idx in
  ...
  let retrieve_result =
    ...
    | Some _, false ->
        let get_int i (t, op) =
          let r = List.nth int_ret_regs i in
          match t with
          | Assembly.ByteArray { size; _ } ->
        ...
        in
        let get_dbl i op =
          let r = List.nth dbl_ret_regs i in
      ...
      if return_on_stack then
        ...
        let return_ints =
          List.concat
            (List.mapi
               (fun i (t, op) ->
                 let dst_reg = List.nth [ AX; DX ] i in
                 match t with
                 | ByteArray { size; _ } ->
        ...
        let return_dbls =
          List.mapi
            (fun i op -> Mov (Double, op, Reg (List.nth [ XMM0; XMM1 ] i)))
            dbl_retvals
        in
...
let pass_params param_list return_on_stack =
  ...
  let pass_in_int_register idx (param_t, param) =
    let r = List.nth remaining_int_regs idx in
    match param_t with
    | Assembly.ByteArray { size; _ } ->
...
let gen (Tacky.Program top_levels) =
  ...
  let constants =
    List.of_seq (Seq.map convert_constant (Hashtbl.to_seq constants))
  in
