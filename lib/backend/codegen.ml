...
let convert_function_call f args dst =
  let reg_args, stack_args = Utils.take_drop 6 args in
...
let pass_params param_list =
  let register_params, stack_params = Utils.take_drop 6 param_list in
...
let gen (Tacky.Program top_levels) =
  let prog = Assembly.Program (List.map convert_top_level top_levels) in
