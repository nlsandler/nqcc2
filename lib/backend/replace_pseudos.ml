  ...
  let new_state =
    {
      ...
      offset_map = String_map.add name new_offset state.offset_map;
    }
  in
...
let replace_operand state = function
  ...
  | PseudoMem (s, offset) -> (
      ...
      | None ->
          (* assign s a stack slot, and add its offset to the offset w/in s to get new operand *)
...
let replace_pseudos_in_tl = function
  | Function { name; global; instructions } ->
      ...
      in
      let init_state =
        { current_offset = starting_offset; offset_map = String_map.empty }
      in
